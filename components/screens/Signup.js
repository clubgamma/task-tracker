...
export default function Signup({ navigation }) {
  ...
  useEffect(() => {
    // will wait for allusers state to change
    ...
    Alert.alert("Success", "Registeration successfull!", [
      {
        text: "ok",
        onPress: () => {
          navigation.navigate("Login", { allusers });
        },
      },
    ]);
  }, [allusers]);
  ...
  const submitHandler = async (user) => {
    ...
    const search = () =>
      allusers.find(
        (singleuser) => singleuser["user"]?.username === user?.username
      );
    found = search();
    ...
    if (
      ...
      if (found) {
        ...
        setallusers((prevallusers) => {
          return [
            ...prevallusers,
            { user, key: Math.random().toString() }, //appending
          ];
        });
      }
    }
